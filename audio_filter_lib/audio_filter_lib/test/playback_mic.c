...
int in_cb(const void *input, void *o, unsigned long fcount, const PaStreamCallbackTimeInfo* c, PaStreamCallbackFlags f, void *user_data)
{
    ...
    SArgs *handle = (SArgs *)user_data; // af_handle_out
    ...
    int16_t *PCM = (int16_t*)malloc(sizeof(int16_t)*fcount);
    memcpy(PCM, input, sizeof(int16_t)*fcount);

    sf_write_short(handle->pBefore, PCM, fcount);
    ...
    sf_write_short(handle->pAfter, PCM, fcount);
...
int out_cb(const void *i, void *output, unsigned long fcount, const PaStreamCallbackTimeInfo* c, PaStreamCallbackFlags f, void *user_data)
{
    ...
    SNDFILE *af_handle = (SNDFILE *)user_data; // af_handle_in
...
int main (int argc, char** argv)
{
    ...
    //±£´æÔ­Ê¼Â¼Òô
    af_origin = sf_open("origin.wav", SFM_WRITE, &origin_info);
    if (af_origin == NULL) {
        puts("Failed to open output file");
        return 1;
    }
    ...
    SArgs *pSArg = (SArgs*)malloc(sizeof(SArgs));
    pSArg->pBefore = af_origin;
    pSArg->pAfter = af_handle_out;

    PaError err = Pa_OpenStream(&adout, NULL, &output, af_info_in.samplerate, frame_size, paNoFlag, out_cb, af_handle_in);
    assert(err == paNoError);

    err = Pa_OpenStream(&adin, &input, NULL, af_info_in.samplerate, frame_size, paNoFlag, in_cb, pSArg);
    assert(err == paNoError);
    ...
    //set_echo_delay_ms(filteraudio, 0);
    /*
     */
